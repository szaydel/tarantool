...
{
	if (op == SOP_NEIGHBOR) {
		// To return element in order of increasing distance from specified point, we build sorted list of R-Tree items
...
R_page* R_page::split_page(branch const& br)
{
	       ...
	       && group_card[1] < card + 1 - min_fill)
	{
		...
		for (i = 0; i < card; i++) {
			if (!taken[i]) {
				int diff = (area(group[0] + b[i].r) - group_area[0])
...
R_page* R_page::insert(rectangle_t const& r, record_t obj, int level)
{
	branch br;
	if (--level != 0) {
		// not leaf page
...
bool R_page::remove(rectangle_t const& r, record_t rec,
